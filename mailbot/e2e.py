...
if __name__ == "__main__":
    ...
    for mailbox in mailbox_list:
        if mailbox in exception_list:
            logger.info(mailbox, "is in the exception list. Skipping...")
            continue
        email_ids = imapService.fetch_email_ids(mailbox)
        for email_id in email_ids:
            email_data: EmailWrapper = imapService.fetch_email(email_id)
            if not email_data:
                logger.info(f"Could not process email ID {email_id}, skipping.")
                continue # Move to the next email
            importance_level: Optional[ImportanceLevel] = cacheService.exists(email_data)
            if importance_level:
                logger.info(f'Email with subject {email_data.subject} already evaluated and marked as {importance_level.value}')
            ...
            if llm_response["importance"] > 0 and llm_response["confidence"] > 0:
                ...
                logger.info(f'Email with subject {email_data.subject} added to cache and moved to {importance.value}')
